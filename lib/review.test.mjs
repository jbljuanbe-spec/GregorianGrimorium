...
test("cuenta aciertos sobre las aplicables, no sobre el total", () => {
  ...
  assert.equal(audit.applicable, 7);
  assert.equal(audit.passed, 7);
});
...
test("un marcador de la fuente no es un íncipit", () => {
  // Caso real del corpus: «-- No Incipit (mode 8)» como título de un Alleluia.
  const audit = auditChant(chant({ incipit: "-- No Incipit (mode 8)" }));
  const check = checkOf(audit, "incipit");
  assert.equal(check.state, "fail");
  assert.match(check.detail, /marcador de la fuente/);
});

test("un íncipit normal pasa", () => {
  assert.equal(checkOf(auditChant(chant()), "incipit").state, "pass");
});
