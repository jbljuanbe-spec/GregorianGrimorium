...
export const CHECKS = {
  ...
  incipit: "El íncipit nombra la pieza y no es un marcador de la fuente",
...
/**
 * Marcadores que la fuente usa cuando no tiene un íncipit que dar. Sirven de
 * hueco en una base de datos, pero puestos como título de una ficha dicen
 * «-- No Incipit (mode 8)», que no nombra nada.
 */
const PLACEHOLDER_INCIPIT = /^\s*(--|no incipit|untitled|sin t[ií]tulo|\?+)|no incipit/i;

/** El íncipit tiene que nombrar la pieza. */
function checkIncipit(chant) {
  const incipit = (chant.incipit ?? "").trim();
  if (incipit.length === 0) return fail("incipit", "no hay íncipit");
  if (PLACEHOLDER_INCIPIT.test(incipit)) {
    return fail("incipit", `es un marcador de la fuente, no un íncipit: «${incipit}»`);
  }
  return pass("incipit", `«${incipit}»`);
}
