...
test("las rúbricas de sobre el pentagrama no se cantan", () => {
  // <alt> lleva indicaciones escénicas, no sílabas. Antes se quitaba la
  // etiqueta y se dejaba el contenido, que quedaba pegado dentro de la
  // palabra: "AdCantor te, Dómine, * leOmnesvávi".
  const body = "Ad<alt>Cantor</alt>(f) te,(h) le<alt>Omnes</alt>(i)vá(i)vi(h)";
  assert.equal(extractText(body), "Ad te, levávi");
  assert.deepEqual(extractRubrics(body), ["Cantor", "Omnes"]);
});

test("rescata la rúbrica completa para no perderla", () => {
  const body = "Re(f)ple(g)<alt>Hic genuflectitur.</alt>(h)";
  assert.deepEqual(extractRubrics(body), ["Hic genuflectitur."]);
  assert.equal(extractText(body), "Reple");
});

test("el marcado tipográfico sí envuelve texto que se canta", () => {
  // <i>, <b>, <sc>, <ul> son de imprenta: el contenido es sílaba cantada.
  assert.equal(extractText("<i>Al</i>(f)le(g)<b>lú</b>(h)ia(g)"), "Allelúia");
});

test("capitular de dos letras con versalitas: AlLELUIA es Alleluia", () => {
  assert.equal(extractText("Al(e)LE(f)LU(h)IA.(i)"), "Alleluia.");
});
